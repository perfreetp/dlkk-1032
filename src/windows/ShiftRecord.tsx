...
import {
  ...
  Badge, Popover, App, Dropdown, Tabs, Progress
} from 'antd';
import {
  ...
  BellOutlined, EyeOutlined, TeamOutlined
...
const ShiftRecordPage: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const processedItems: HandoverItem[] = pendingHandoverItems.map(item => ({
        ...item,
  ...
  const renderFollowupView = () => {
    ...
    const nurseTracking = useMemo(() => {
      const map = new Map<string, {
        ...
        attention: number;
        toReminder: number;
        items: typeof flatHandoverItems;
      }>();
      for (const item of flatHandoverItems) {
        const nurse = item.item.assignedTo || item.shiftRecord.oncomingNurse || '未分配';
        if (!map.has(nurse)) {
          map.set(nurse, {
            nurse,
            total: 0,
        ...
        const entry = map.get(nurse)!;
        const isDone =
          item.item.status === 'completed' ||
          item.relatedReminder?.status === 'completed';
        entry.total++;
        if (isDone) entry.completed++;
        else entry.pending++;
        if (item.item.status === 'attention') entry.attention++;
        if (item.relatedReminder) entry.toReminder++;
    ...
    return (
      <div>
        ...
        <Card
          ...
          title={
            <Space>
              <TeamOutlined /> 责任护士追踪面板
              <Tag color="blue">{nurseTracking.length} 位护士</Tag>
            </Space>
          }
          size="small"
        >
          {nurseTracking.length === 0 ? (
            <Empty description="暂无责任护士分配" image={Empty.PRESENTED_IMAGE_SIMPLE} style={{ padding: 20 }} />
          ) : (
            <Row gutter={[12, 12]}>
              {nurseTracking.map(entry => {
                const pendingRate = entry.total > 0 ? Math.round(entry.completed / entry.total * 100) : 0;
                return (
                  <Col xs={24} sm={12} md={8} lg={6} key={entry.nurse}>
                    <div
                      style={{
                        padding: 12,
                        borderRadius: 10,
                        background: entry.pending > 0
                          ? 'linear-gradient(135deg, #fff7e6 0%, #fff1f0 100%)'
                          : 'linear-gradient(135deg, #f6ffed 0%, #e6fffb 100%)',
                        border: `1px solid ${entry.pending > 0 ? '#ffd591' : '#b7eb8f'}`,
                        cursor: 'pointer',
                        transition: 'all 0.2s'
                      }}
                      ...
                      onClick={() => {
                        const firstRem = entry.items.find(i => i.relatedReminder);
                        const firstItem = entry.items[0];
                        if (firstRem && firstRem.relatedReminder) {
                          goToReminder(firstRem.relatedReminder, firstRem.item.babyId);
                        } else if (firstItem && firstItem.baby) {
                          setSelectedBaby(firstItem.baby);
                      ...
                      <div style={{
                        ...
                        <Space>
                          <Avatar size={32} style={{
                            ...
                            {entry.nurse.charAt(0)}
                          </Avatar>
                          <div>
                            <div style={{ fontWeight: 600, fontSize: 13 }}>👩‍⚕️ {entry.nurse}</div>
                            <div style={{ fontSize: 10, color: '#999' }}>责任护士</div>
                          </div>
                        </Space>
                        <Space direction="vertical" size={2} style={{ textAlign: 'right' }}>
                          {entry.attention > 0 && (
                            <Tag color="red" style={{ margin: 0, fontSize: 10 }}>
                              ⚠️ {entry.attention} 重点
                            </Tag>
                          )}
                          <Tag color={entry.pending > 0 ? 'orange' : 'green'} style={{ margin: 0, fontSize: 10 }}>
                            {entry.pending > 0 ? `⏳ ${entry.pending} 待办` : `✅ 全部完成`}
                          </Tag>
                        </Space>
                      </div>
                      <Progress
                        percent={pendingRate}
                        size="small"
                        strokeColor={pendingRate === 100 ? '#52c41a' : '#fa8c16'}
                        format={(p) => `${p}%`}
                      />
                      <div style={{
                        display: 'flex',
                        ...
                        <span>📋 共{entry.total}项</span>
                        <span>✅ {entry.completed}完成</span>
                        <span>🔔 {entry.toReminder}提醒</span>
                      </div>
                    </div>
                  </Col>
                );
